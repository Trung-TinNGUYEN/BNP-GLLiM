...
from vbem_utils_numba import init_by_kmeans, init_by_gmm, compute_weighted_mean, \
    compute_weighted_cov, compute_niw, expectation_log_gauss, compute_exp_inv_psi,\
...
class Gaussian(Distribution):
    ...
    def init(self, data_X, data_Y, nb_data, dim_data_X, dim_data_Y, k_trunc, seed, nbinit):
        """
        ...
        m_niw_0, lambda_niw_0, psi_niw_0, nu_niw_0, q_z, labels = init_by_gmm(data, 
                                     nb_data, dim_data, data_X, dim_data_X, k_trunc,
                                     nbinit, seed=seed)        
	   ...
	   # Markov Random field parameters initialization
        ...
        return q_z, s1, s2, mean_sigma, beta_z_0, A_hat_init, b_hat_init, Sigma_hat_init,\
    ...
    def update_posterior(self, data_X, q_z, nk, nb_data, dim_data_X, k_trunc):
        ...
        weighted_mean = compute_weighted_mean(q_z, data_X, nk, nb_data, 
                                              dim_data_X, k_trunc)
        weighted_cov = compute_weighted_cov(q_z, data_X, nk, weighted_mean, 
...
# TrungTin Nguyen.  [Eq. (59) Lu et al., 2020)].
    def expectation_likelihood(self, data_X, data_Y, nb_data, dim_data_X, dim_data_Y, k_trunc):
        ...
        e = expectation_log_gauss(data_X, m_niw, lambda_niw, psi_niw, nu_niw,
    ...
    def convergence(self, model, delta_beta, delta_qz, threshold, spatial_interaction):
        
        if (model == 'dp-mrf' or model == 'pyp-mrf') and (spatial_interaction != 0.0):
            return delta_qz < threshold or delta_beta == 0.0
        else:
