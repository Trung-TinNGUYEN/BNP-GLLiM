...
__ Updated for Python 3 (2022/09/26): TrungTin Nguyen.

Reference:  Deep Mixture of Linear Inverse Regressions Applied to Head-Pose Estimation,
Stéphane Lathuilière, Rémi Juge, Pablo Mesejo, Rafael Muñoz Salinas, Radu Horaud, CVPR 2017.
...
Correct:
    ...
    --> Changed: self.rnk = np.empty((1,1)) # need to be allocated
"""
...
## logsumexp moved from scipy.misc to scipy.special starting from scipy version '1.0.0'
...
class GLLIM:
    ...
    def __init__(self,K_in,D_in,L_in,N_in):
    ...
    def fit(self, X, Y, maxIter, init, gmm_init = None, random_state = None):
        '''fit the Gllim
           # Arguments
        ...
        if init==True:
            ...
            # gmm = GaussianMixture(n_components=self.K, covariance_type='diag', random_state=None, tol=0.001, min_covar=0.001, n_iter=100, n_init=3, params='wmc', init_params='wmc', verbose=1)
            # TestT 
            gmm = GaussianMixture(n_components=self.K, covariance_type='diag', 
                                  random_state=None, tol=0.001, n_init=3,
        ...
        ## TestT
        # rnk_test = self.rnk
        # print('Shape rnk = ', rnk_test.shape)
        rkList = [np.sum(self.rnk[:,k]) for k in range(self.K)]
        ...
        while (converged==False) and (it<maxIter):
            ...
            # isotropic sigma
            # self.SigmakSquareList = [(np.sum(sig2+sys.float_info.epsilon)/self.D) for sig2 in sigma2] 
            # TestT
            self.SigmakSquareList = [(np.sum(sig2+sys.float_info.epsilon)/self.D) for sig2 in sigma2] 
            # numerical stability
        ...
        return LL[1:it,0], self.pikList, rkList,  self.rnk
    ...
    def inversion(self):
        ...
        ## Raise LinAlgError("Singular matrix"): 
            ## self.SigmakListS[:]=[inv(inv(gam)+(Ak.T).dot(1/sig*Ak)) for sig,gam,Ak
        self.GammakListS[:]=[sig*np.eye(self.D)+Ak.dot(gam).dot(Ak.T) for sig,gam,Ak \
                             in zip(self.SigmakSquareList,self.GammakList,self.AkList)]  # (10)
        
        self.SigmakListS[:]=[inv(inv(gam)+(Ak.T).dot(1/sig*Ak)) for sig,gam,Ak \
                             in zip(self.SigmakSquareList,self.GammakList,self.AkList)]  # (14)
        
        self.AkListS[:]=[sigS.dot(1/sig*Ak.T) for Ak,sigS,sig in zip(self.AkList,
                                                                     self.SigmakListS,\
                                                                         self.SigmakSquareList)]  # (12)
        
        self.bkListS[:]=[sigS.dot(inv(gam).dot(ck)-(1/sig*Ak.T).dot(bk))\
                         for Ak,bk,ck,sig,sigS,gam in zip(self.AkList,self.bkList,\
                                                          self.ckList,self.SigmakSquareList,\
                                                              self.SigmakListS,self.GammakList)]  # (13)
    ...
    def predict_high_low(self,Y):
        ...
        for (k,pik,AkS,bkS,ckS,gamkS) in zip(range(self.K),self.pikList,self.AkListS,\
