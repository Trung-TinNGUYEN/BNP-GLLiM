...
###############################################################################
####                        Functions for conditional density
###############################################################################
...
def compute_z_tau_entropy(beta_z, q_z, taus, neighborhood, nb_contrib):
    ...
    nb_data, k_trunc, nk, entropy = compute_pi_tau_entropy(q_z, taus)
...
#
def inversion(A_hat, b_hat, Sigma_hat, c_hat, Gamma_hat):
    # Initialization:
    k_trunc, dim_data_Y, dim_data_X,  = A_hat.shape
    ...
    for k in range(k_trunc):
        ...
        Sigma_hat[k] += sys.float_info.epsilon*np.diag(np.ones(Sigma_hat[k].shape[0]))
        Gamma_hat[k] += sys.float_info.epsilon*np.diag(np.ones(Gamma_hat[k].shape[0])) 
    ...
    return A_hatS, b_hatS, Sigma_hatS, c_hatS, Gamma_hatS

def inverse_density_true(X, Y, K_true, pi_true, A_true, b_true, Sigma_true, 
                         c_true, Gamma_true):

    (nb_data, dim_data_X) = np.shape(X)
      
    ## Calculate the true gating networks.
    gk_true = posterior_Gaussian_gate(X, pi_true, c_true, Gamma_true)[0]
    
    pyx_true = np.zeros((nb_data, 1), dtype=np.float64)
    
    for n in range(nb_data):
        for k in range(K_true):
            pyx_true[n,:,None] += gk_true[n,k] *\
                multivariate_normal.pdf(Y[n,:,None],
                                           A_true[k,:,:]@(X[n,:,None].T) + b_true[k,:].T,
                                           Sigma_true[k,:,:]) 
    return pyx_true

def gate_weight_post(k_trunc, gamma_sb):
    '''
    Calculate the gating weight posteriors E_{q_tau}[pi_k(tau)]

    Parameters
    ----------
    k_trunc : int
        Truncation values of nb clusters. 
    gamma_sb : ((k_trunc, 2) np.array)
        Variational parameters gamma of tau posteriors.

    Returns
    -------
    gate_weight_post : ((k_trunc, 1) np.array)
        Gating weight posteriors E_{q_tau}[pi_k(tau)]

    '''
    gate_weight_post = np.zeros((k_trunc, 1), dtype=np.float64)
    # for k in range(1,k_trunc):
    #     prod_temp = 1.0
    #     for l in range(1,k):
    #         prod_temp *= gamma_sb[l-1,1]/(gamma_sb[l-1,0]+gamma_sb[l-1,1])      
    #     gate_weight_post[k-1,0] = gamma_sb[k-1,0]/(gamma_sb[k-1,0]+gamma_sb[k-1,1])*prod_temp
    
    # TestT
    gate_weight_post[0] = gamma_sb[0,0]/(gamma_sb[0,0]+gamma_sb[0,1])   
    for k in range(1,k_trunc):
        prod_temp = 1.0
        for l in range(0,k):
            prod_temp *= gamma_sb[l,1]/(gamma_sb[l,0]+gamma_sb[l,1])      
        if k == (k_trunc-1):
            gate_weight_post[k,0] = prod_temp
        else:
            gate_weight_post[k,0] = gamma_sb[k,0]/(gamma_sb[k,0]+gamma_sb[k,1])*prod_temp
        
    return gate_weight_post
...
def compute_abs(q_z, data, data_Y, nk, normalizing_weighted_input,
    ...
    for k in range(k_trunc):
        ...
        ## Pour tester.
        # print('XkXk^T = ', 
        #       normalizing_weighted_input[k]@(normalizing_weighted_input[k].T))
        # print('Singular normalizing_weighted_input = ', 
        #       normalizing_weighted_input[k])
        # print('qz for Singular matrix = ', np.sum(q_z, axis=0)[k])
        # print('qz for Singular matrix = ', np.sum(q_z))
        # updatek = [True]*A_hat.shape[0]
        # if (np.sum(normalizing_weighted_output[k]) == 0)or(np.sum(q_z, axis=0)[k] < 1):
        #     print('Empty cluster = ', k)
        
        Xk = normalizing_weighted_input[k]@(normalizing_weighted_input[k].T) +\
            sys.float_info.min
        Yk = normalizing_weighted_output[k]@(normalizing_weighted_input[k].T)
        # A_hat[k] = normalizing_weighted_output[k]@(normalizing_weighted_input[k].T)\
        #     @np.linalg.inv(normalizing_weighted_input[k]@(normalizing_weighted_input[k].T))
        A_hat[k] = Yk@np.linalg.inv(Xk)
    ...
    ### Showing Sigma,A_hat,b_hat for each iteration.
    # print('Sigma_hat_test = ', Sigma_hat)
    # print('A_hat_test = ', A_hat)
    # print('b_hat_test = ', b_hat)
    # print('nk_test = ', nk)
...
#@nb.jit(nopython=True, nogil=True, fastmath=True, parallel=False)
def compute_exp_inv_Gamma(data_X, c_hat, Gamma_hat, 
    ...
    data_X : ([nb_data, dim_data_X] np.array)
        Input sample.    
    c_hat : ([k_trunc, dim_data_X] np.array)
        location parameters of normal inverse Wishart.
    Gamma_hat : ([k_trunc] np.array)
    ...
    Returns
    -------
    inv_Gamma : ([nb_data, k_trunc] np.array)
    
    """

    inv_Gamma = np.zeros((nb_data, k_trunc), dtype=np.float64)
    
    for k in range(k_trunc):
        Gamma_hat_inv = np.linalg.inv(Gamma_hat[k]+sys.float_info.epsilon*np.diag(np.ones(Gamma_hat.shape[1])))
        for n in range(nb_data):
            for d1 in range(dim_data_X):
                for d2 in range(dim_data_X):
                    inv_Gamma[n, k] += (data_X[n, d1] - c_hat[k, d1]) \
                        * Gamma_hat_inv[d1, d2] \
                        * (data_X[n, d2] - c_hat[k, d2])
...
def compute_exp_inv_Sigma(data_X, data_Y, A_hat, b_hat, Sigma_hat, nb_data, dim_data_Y, k_trunc):
    ...
    for k in range(k_trunc):
        Sigma_hat_inv = np.linalg.inv(Sigma_hat[k] + sys.float_info.epsilon*np.diag(np.ones(Sigma_hat.shape[1])))
        for n in range(nb_data):
            for d1 in range(dim_data_Y):
                for d2 in range(dim_data_Y):
...
def expectation_log_gauss(data_X, c_hat, Gamma_hat, nb_data, dim_data_X, 
    ...
    inv_Gamma = compute_exp_inv_Gamma(data_X, c_hat, Gamma_hat,nb_data,
                                      dim_data_X, k_trunc)
    inv_Sigma = compute_exp_inv_Sigma(data_X, data_Y, A_hat, b_hat, Sigma_hat,
                                      nb_data, dim_data_Y, k_trunc)
    
    log_cov_Gamma = np.zeros((1, k_trunc), dtype=np.float64)
    log_cov_Sigma = np.zeros((1, k_trunc), dtype=np.float64)   
    for k in range(k_trunc):   
        det_Gamma = max(np.linalg.det(Gamma_hat[k]), 1e-300)
        # ld = math.log(det_Gamma)
        det_Sigma = max(np.linalg.det(Sigma_hat[k]), 1e-300)
        # aux = 0.0
        # for d in range(dim_data_X):
        #     aux += special.digamma((nu_niw[k]-d)*0.5)
        log_cov_Gamma[0, k] = math.log(det_Gamma) + dim_data_X*np.log(2*np.pi)
        log_cov_Sigma[0, k] = math.log(det_Sigma) + dim_data_Y*np.log(2*np.pi)

    log_gauss = log_cov_Sigma + inv_Sigma + log_cov_Gamma + inv_Gamma
...
def expectation_z(q_z, beta_z_0, exp_log, log_tau, log_one_minus_tau, 
    ...
    for n in range(nb_data):
        ...
        q_z_new[n, :] = exp_log[n, :] + log_tau_k_trunc + log_one_minus_tau_aux + local_energy
...
def update_gamma_sb(nk, mean_alpha, mean_sigma, k_trunc):
    ...
    :Returns:
        Variational parameters gamma of tau posteriors ([k-1, 2] np.array)
    """
    gamma_sb = np.zeros((k_trunc-1, 2), dtype=np.float64)
    for k in range(k_trunc-1):
        sum_aux = 0.0
...
def init_by_gmm(data, nb_data, dim_data, data_X, dim_data_X, k_trunc, nb_init, seed=100):
    """
    Initialize variational q_z by k-means using the link 
    between Joint-GMM and GLLiM run EMGM (EM algorithm for fitting the
...
def save_fig(fname, *args, **kwargs):
    ...
    figdir = os.path.join(current_dir, "..", "figures_simplest")
